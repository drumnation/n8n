...
export const run = async (testType?: string) => {
  ...
  console.log(
    `🧪 Running ${getTestDisplayName(actualTestType as any)} with turbo...`,
  );
  console.log(
    `📦 Found ${packagesWithTest.length} packages with ${actualTestType}`,
  );
  console.log(
  ...
  testProcess.stdout?.on("data", (data) => {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      ...
      if (failMatch) {
        ...
        if (
          testFullName.includes(".test.") ||
          testFullName.includes(".spec.")
        ) {
